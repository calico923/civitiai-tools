#!/usr/bin/env python3
"""
殿堂入り＋Top100の最終HTMLレポートを生成するスクリプト
"""
...
def create_html_report():
    # データを読み込み
    hall_of_fame_df = pd.read_csv('/Users/kuniaki-k/Code/civitiai/outputs/enhanced/illustrious_checkpoint_hall_of_fame_final.csv')
    top100_df = pd.read_csv('/Users/kuniaki-k/Code/civitiai/outputs/enhanced/illustrious_checkpoint_top100_excluding_hall_of_fame.csv')
    
    # 統計情報を計算
    hof_model = hall_of_fame_df.iloc[0]
    sfw_count = (~top100_df['nsfw']).sum()
    nsfw_count = top100_df['nsfw'].sum()
    total_models = len(top100_df)
...
<head>
    ...
    <title>Illustrious Checkpoint 殿堂入り＋Top100 ランキング</title>
    <style>
        body {{ 
...
<body>
    <div class="container">
        <div class="update-notice">
            🏆 最終版！殿堂入り枠を新設し、残り635モデルからTop100を選出
        </div>
        
        <h1>🏆 Illustrious Checkpoint 殿堂入り＋Top100 ランキング</h1>
        ...
        <div class="hall-of-fame">
            ...
            <div class="model-info">
                ...
                <div class="metric">
                    <div class="metric-value">{hof_model['engagement_score']:,}</div>
                    <div>エンゲージメント</div>
                </div>
            </div>
        ...
        <div class="stats">
            <div class="stat-item">
                ...
                <div class="stat-label">ランキング<br>モデル数</div>
            </div>
            <div class="stat-item">
                <span class="stat-value">635</span>
                <div class="stat-label">対象<br>モデル数</div>
            </div>
            <div class="stat-item">
                <span class="stat-value">{top100_df.iloc[0]['thumbs_up_count']:,}</span>
                <div class="stat-label">最高いいね数<br>(1位)</div>
            </div>
            <div class="stat-item">
                <span class="stat-value">{top100_df['updated_download_count'].max():,}</span>
                <div class="stat-label">最高DL数</div>
            </div>
            <div class="stat-item">
                <span class="stat-value">{len(top100_df['creator'].unique())}</span>
    ...
    html_content += f"""
        ...
        <div class="methodology">
            ...
            <ul>
                ...
                <li><strong>対象モデル:</strong> 'base model'タグを持つCheckpointから殿堂入りを除いた635モデル</li>
                <li><strong>スコア計算:</strong> いいね数(40%) + ダウンロード数(30%) + エンゲージメント(20%) + コメント(10%)</li>
                <li><strong>正規化:</strong> 635モデルの範囲で0-100点に正規化し、上位100モデルを選出</li>
                <li><strong>NSFW判定:</strong> タグベース分類（nsfw, hentai, sexy, porn等）</li>
                <li><strong>データソース:</strong> CivitAI API v1 個別モデルAPIから詳細メトリクス取得</li>
            </ul>
...
def main():
    ...
    output_file = '/Users/kuniaki-k/Code/civitiai/outputs/enhanced/illustrious_checkpoint_final_hall_of_fame_top100_report.html'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"最終HTMLレポートを生成しました: {output_file}")
