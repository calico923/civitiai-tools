#!/usr/bin/env python3
"""Test streaming search for Concept category - 200 models (FIXED VERSION)."""
...
async def test_concept_200():
    """Test streaming search for Concept category - 200 models."""
    print("💡 CONCEPT CATEGORY - 200 MODELS TEST (FIXED VERSION)")
    print("=" * 60)
    ...
    # Search parameters for Concept category
    search_params = AdvancedSearchParams(
        query='/',
        model_types=['LORA'],
        base_model='Illustrious',
        categories=[ModelCategory('concept')],
    ...
    try:
        ...
        print(f"\n🎯 Concept category accuracy: {target_category_count}/{model_count} ({accuracy:.1f}%)")
        ...
        print(f"❌ Error in Concept test: {e}")
        import traceback
        traceback.print_exc()
        return {'success': False, 'error': str(e)}

if __name__ == "__main__":
    result = asyncio.run(test_concept_200())
